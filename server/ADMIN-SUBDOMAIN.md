...
Всё делает один скрипт. На сервере:

```bash
ssh -p 2222 ubuntu@167.172.191.171
cd /srv/gamesite && ./deploy.sh back
sudo bash /srv/gamesite/server/setup-admin-subdomain.sh
```

Спросит один раз — пароль для nginx. Дальше сам: проверит DNS, закроет
`/admin/` на публичном домене, поднимет поддомен, выпустит сертификат,
пропишет переменные в `.env`, перезапустит Django и прогонит проверки.

Перед каждым шагом складывает копии конфигов в `/root/admin-subdomain-backup-*`.
Если `nginx -t` или `manage.py check` не проходят — откатывает сам и оставляет
сайт в рабочем виде. Запускать повторно безопасно: что уже сделано, пропустит.

Другой домен или логин:

```bash
sudo DOMAIN=example.com ADMIN_USER=admin bash /srv/gamesite/server/setup-admin-subdomain.sh
```
...
Нужно, только если скрипт где-то споткнулся и надо разобраться.
